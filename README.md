...
## Bidding
:::warning
:warning: Only registered bidders can participate in the auction. Operators can onboard new bidders through the contract. Please contact Mev Protocol to proceed.
:::
...
cast call $AUCTIONEER \
    "packBid(uint128,uint120,uint8)" 0.01gwei 21000 \
    $(cast call $AUCTIONEER "IdMap(address)" $WALLET_ADDRESS --rpc-url $L2_RPC_URL) \
    --rpc-url $L2_RPC_URL

# Placing a bid
cast send $AUCTIONEER \
    "bid(uint256,uint256[] memory)" SLOT_NUMBER ["0x0000000000000000000000000098968000000000000000000000000000520852"]
```
